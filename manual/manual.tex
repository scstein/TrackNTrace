...
\begin{itemize}[leftmargin=2cm]
\item[Loop direction] Movies can be processed in a forward direction, from beginning to end, or backwards. Normally, there is no difference between the two. However, this setting can be used in combination with \textsf{Calculate candidates once} to only track emitters which are on in the first or last frame, useful for drift correction or blinking/bleaching analysis.
\item[\smash{\tshortstack[1]{Calculate candi-\\dates once}}] If enabled, spots are only detected in the first (or last, with \textsf{Loop direction = Backward}) frame. To increase the signal-to-noise ratio for this step, it is possible to average over a number of frames next to the first or last one.
