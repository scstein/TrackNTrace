...
plugin.add_param('scalingFactor',...
    'float',...
    {2, 0, inf},...
    'Wavelet scaling factor. This determines how fast the filter falls of at the edges. Lower = greater falloff, which means sharper, more insular features. Values lower than 2 tend to find single pixels as candidates which is not recommended.');
plugin.add_param('splineOrder',...
    'int',...
    {3,1,inf},...
    'B-spline order. Determines how smooth the filter falls off towards the edge. Higher = smoother falloff. Higher values mean one tends to find larger, "flatter" structures.');
