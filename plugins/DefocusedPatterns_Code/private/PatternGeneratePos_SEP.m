function [int,Sx,Sy] = PatternGeneratePos_SEP(xcent,ycent,z, NA, n0, n, n1, d0, d, d1, lamem, mag, focus, atf, ring, pixel, nn, field, be, al, pic, SEPdata)
...
% p = angle( (x-rem(xcent,1)) +1i*(y-rem(ycent,1)));% subpixel shift!
% r = sqrt((x-rem(xcent,1)).^2+(y-rem(ycent,1)).^2);% subpixel shift!
p = angle(x+1i*y);
r = sqrt(x.^2+y.^2);
rho = rho/pixel;
...
% Shift using subindexing (careful for negative shifts / outside image shifts)
% xshift = field(2)-floor(size(int,2)/2)+fix(xcent);
% yshift = field(1)-floor(size(int,1)/2)-fix(ycent);
% tmpint(yshift+1:yshift+size(int,2), xshift+1:xshift+size(int,1)) = int; % Problem for shifts outside image!
% int = tmpint;
...
% tmpint = zeros(size(Sx));
% wx = ismember(Sx,x); wy = ismember(Sy,y);
% tmpint(logical(wx.*wy))  = int.*bck;
% int = tmpint;
