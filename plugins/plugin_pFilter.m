...
plugin.info = 'Finds candidates based on a p-Value hypothesis test against local background.';
...
plugin.add_param('pThreshold',...
    'float',...
    {0.95, 0, 1},...
    'p-Value threshold above which pixels can be identified as candidates. Higher means less but higher quality candidates.');
...
plugin.add_param('clusterSize',...
    'int',...
    {4,2,inf},...
    'Number of pixels belonging to a local maximum candidate. Should be similar to the particle size.');
end
