function [plugin] = plugin_fitLT_spectralSplit()

%    -------------- Definition of plugin --------------
% Name of the component these options are for
name = 'fit lifetime + spectral splitting';
...
% Description of output parameters. This is overwritten by the init function
...
% parallel execution.
    ...
    options.outParamDescription = trackingOptions.outParamDescription(:);
    ...
    elseif options.hasTCSPC
        ...
        switch options.position
            case 'Refit'
                ...
                halfw = max(round(3*quantile(trackingData(:,8),0.99)),4)+1; % for dense data this window might be too large
                imgLoc = zeros(2*halfw+1,2*halfw+1,trackingData(end,1),size(img,3));
                ...
                imgLoc = reshape(imgLoc,size(imgLoc,1),[],size(imgLoc,4));
                % refit positions
                rewindMessages();
                rewPrintf('TNT: Refitting positions of tracks: performing fit\n');   
                
                % 'optimize x,y,A,BG,sigma' (isoptric gaussian),integrated=true,MLE=true,halfw
                newLoc = psfFit_Image(sum(imgLoc,3),oldLoc(:,[1,2,4,5,6])',[1,1,1,1,1,0,0],true,true,halfw-1);
                rewindMessages();
                rewPrintf('TNT: Refitting positions of tracks: calculating amplitude ratio\n');
                
                % remove values where the fit failed or the position changed too much (>2px).
                newLoc_valid = newLoc(end,:)==1 & sum((oldLoc(:,1:2)'-newLoc(1:2,:)).^2,1)<2^2;
                
                chanAmp = nan(size(newLoc,1),size(newLoc,2),size(imgLoc,3));
                for cidx = 1:size(imgLoc,3)
                    chanAmp(:,newLoc_valid,cidx) = psfFit_Image(imgLoc(:,:,cidx),newLoc(1:5,newLoc_valid),[0,0,1,1,0,0,0],true,true,halfw-1);
                end
                % remove values where one of the channel fits failed.
                newLoc_valid = sum(chanAmp(end,:,:),3)>1;
                chanAmp(:,~newLoc_valid,:) = NaN;
                % set negative amplitudes to 0 (alternatively we could remove these values)
                chanAmp(3,:,:) = max(chanAmp(3,:,:),0);
                ...
                newLoc = [newLoc(1:2,:);zeros(1,size(newLoc,2));newLoc(3:4,:)./track_len';newLoc(5,:);chanAmp(3,:,1)./sum(chanAmp(3,:,:),3)]; % Add z, normalise amplitude and background to number of frames and add amplitude ratio
                trackingData = [trackingData(:,1:2),newLoc(1:end-1,track_ind)',trackingData(:,9:end),newLoc(end,track_ind)'];
                % Remove failed fits
                ...
                options.outParamDescription = [options.outParamDescription;{'amp-ratio'}];
            case 'Average'
...
%         tau_fast = real(sqrt(((squeeze(sum(t.^2.*double(tcspc_mol),3)))-((squeeze(sum(t.*double(tcspc_mol),3))).^2)./nphot)./(nphot-1))); % Normalise with (N-1) to make it consitent with (my)var
        ...
        tcspc_mol = permute(tcspc_mol(:,:,:,:),[2 3 4 1]);%[track gate chan]
        
        options.outParamDescription = [options.outParamDescription;{'tau-fast';'nphoton'}];
        tcspc_mol_sum = sum(tcspc_mol,3);
        nphot = sum(tcspc_mol_sum,2);
        t = (1:size(tcspc_mol_sum,2))*resolution; % 1D time axis in the 2nd dimension.
        tau_fast = real(sqrt(((squeeze(sum(t.^2.*double(tcspc_mol_sum),2)))-((squeeze(sum(t.*double(tcspc_mol_sum),2))).^2)./nphot)./(nphot-1))); % Normalise with (N-1) to make it consitent with (my)var
        clear tcspc_mol_sum;
        
        options.outParamDescription = [options.outParamDescription;{'nphoton-ratio'}];
        nphot_ratio = sum(tcspc_mol(:,:,1),2)./sum(sum(tcspc_mol,3),2);
        
        % Align the channels, cut tcspc and genetate time axis
        [tcspc_cut,t,tail_ind] = tcspc_apply_cutoff(tcspc_mol,options.cutoff,resolution);        
        tcspc_cut = sum(tcspc_cut,3); % For now use the sum of all channels
        % fit
        ...
        switch options.fitType
            case {'monoexponential MLE','monoexponential PM'}
                ...
                % set cases were we have no information to nan
            ...
            case 'maximum correlation'
                ...
                allcorr = corr(tcspc_cut(fitind,:)',taus_z);
        ...
        if options.sumTCSPC
            if isempty(fitData)
                postprocData = [trackingData, tau_fast(trackingData(:,1),1), nphot(trackingData(:,1),1), nphot_ratio(trackingData(:,1),1)];
            else
                postprocData = [trackingData, tau_fast(trackingData(:,1),1), nphot(trackingData(:,1),1), nphot_ratio(trackingData(:,1),1), fitData(trackingData(:,1),:)];
            end
        else
            postprocData = [trackingData, tau_fast(:), nphot(:), nphot_ratio(:), fitData];
        end
...
function [tcspc,t,ind] = tcspc_apply_cutoff(tcspc,cutoff,resolution)
    ...
    tcspc_int = sum(tcspc,2);
    int_thres = quantile(tcspc_int,[0.25; 0.75]);
    for cidx = 1:size(int_thres,3)
         [~,max_pos(cidx)] = max(sum(tcspc(int_thres(1,:,cidx) <= tcspc_int(:,:,cidx) & tcspc_int(:,:,cidx) <= int_thres(2,:,cidx),:,cidx),1));
         tcspc(:,:,cidx) = circshift(tcspc(:,:,cidx),max_pos(1)-max_pos(cidx),2);
    end
    ind = (max_pos(1)+cutoff):size(tcspc,2);
    tcspc = tcspc(:,ind,:);
