function [movieInfo,exceptions,localMaxima,background,psfSigma] = ...
    detectSubResFeatures2D_StandAlone(imageStack,movieParam,detectionParam,verbose)
...
firstImageNum = 1;
lastImageNum = size(imageStack,3);
...
[imageSizeX,imageSizeY,~] = size(imageStack);

%calculate background properties at movie end
imageLast5 = imageStack(:,:,end-4:end);
...
imageStack = imageStack/(2^bitDepth-1);
parfor iImage = 1:lastImageNum
    imageRaw = imageStack(:,:,iImage);
    ...
    imageIntegF = filterGauss2D(imageRaw,psfSigma);
    ...
    [bgMeanInteg,bgStdInteg] = ...
        spatialMovAveBG(imageRaw,imageSizeX,imageSizeY);
...
%     end
    ...
    %display progress
    if verbose
        progressText(iImage/lastImageNum,['Detecting local maxima with integration window = ' num2str(integWindow(iWindow))]);
    end
...
compareImages = goodImages(:)';
%go over all non-empty images ...

parfor iImage = 1:lastImageNum
    if sum(iImage==compareImages) >=1
        ...
        try %try to detect features in this frame
            ...
            if isempty(featuresInfo.xCoord)
                emptyFrames = [emptyFrames; iImage];
            end
            ...
            %fitting
            framesFailedMMF = [framesFailedMMF; iImage];
