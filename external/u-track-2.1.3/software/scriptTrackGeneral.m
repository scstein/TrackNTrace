...
files_list = dir(saveResults.dir);
files_list = files_list(find(cellfun(@(var) ~isempty(strfind(var,'_res')),{files_list(:).name})));

for i=9:size(files_list,1)
    saveResults.filename = [files_list(i).name(1:end-4),'_tracks'];
    load([saveResults.dir,files_list(i).name]);
    
    for j=1:size(movieInfo,1)
        if ~isempty(movieInfo(j).amp) && size(movieInfo(j).amp(:,1),1)>20
            [~,idx] = sort(movieInfo(j).amp(:,1),'descend');
            movieInfo(j).xCoord = movieInfo(j).xCoord(idx(1:end-7),:);
            movieInfo(j).yCoord = movieInfo(j).yCoord(idx(1:end-7),:);
            movieInfo(j).amp = movieInfo(j).amp(idx(1:end-7),:);
        end
    end
    
    q = round(size(movieInfo,1)/2);
    
    [tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparse(movieInfo(1:q),...
        costMatrices,gapCloseParam,kalmanFunctions,probDim,saveResults,verbose);
    
    traj = [];
    for j=1:size(tracksFinal,1)
        tridx = [tracksFinal(j).seqOfEvents(1,1),tracksFinal(j).seqOfEvents(end,1),tracksFinal(j).seqOfEvents(end,1)-tracksFinal(j).seqOfEvents(1,1)];
        xyatemp = [tracksFinal(j).tracksCoordAmpCG(1:8:end).',tracksFinal(j).tracksCoordAmpCG(2:8:end).',tracksFinal(j).tracksCoordAmpCG(4:8:end).'];
        traj = [traj;[repmat(j,size(xyatemp,1),1),xyatemp(:,[3,1,2])]];
    end
    last_traj = j;
    
    [tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparse(movieInfo(q+1:end),...
        costMatrices,gapCloseParam,kalmanFunctions,probDim,saveResults,verbose);
    for j=1:size(tracksFinal,1)
        tridx = [tracksFinal(j).seqOfEvents(1,1),tracksFinal(j).seqOfEvents(end,1),tracksFinal(j).seqOfEvents(end,1)-tracksFinal(j).seqOfEvents(1,1)];
        xyatemp = [tracksFinal(j).tracksCoordAmpCG(1:8:end).',tracksFinal(j).tracksCoordAmpCG(2:8:end).',tracksFinal(j).tracksCoordAmpCG(4:8:end).'];
        traj = [traj;[repmat(j+last_traj,size(xyatemp,1),1),xyatemp(:,[3,1,2])]];
    end
    
    save([saveResults.dir,saveResults.filename],'traj');
end
