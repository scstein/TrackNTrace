...
% Jean-Yves Tinevez < jeanyves.tinevez@gmail.com> November 2011 - 2012
  ...
  %% Originial serial version of computing the distance matrix
...
%     end
  ...
  %% Simons parallel version of computing the distance matrix..
        % Compute a^2 and b^2
     source_NormVec = sum(source.^2,2);
     target_NormVec = sum(target.^2,2);

     sourceNormMat = repmat(source_NormVec,  1, size(target,1) );
     targetNormMat = repmat(target_NormVec',  size(source,1), 1);

     MixTermMat = source*target'; % Compute MixTerm

     % Overall euclidean distance squared
     D = sourceNormMat + targetNormMat - 2*MixTermMat;    
  %%
