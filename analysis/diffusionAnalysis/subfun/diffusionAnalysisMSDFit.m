function [D_final,velocity_final] = diffusionAnalysisMSDFit(msd_result,experimParam,fitParam,printParam)
...
% 
% INPUT:
%     msd_result: [1xD] cell array D is the dimension. Every cell contains
%     a double array
...
%     
% OUTPUT:
%     D_final: Struct array for diffusion coefficients. It contains a
%     double array for D, the MSD y-intersection and the optimal number of
%     fitting parameters. The first line contains the values, the second
%     line the errors. D is in [µm^2/s].
% 
%     velocity_final: Struct array for velocities. v is in [µm/s].


D_final = struct('D_x',[],'MSD_t0_x',[],'N_opt_x',[],'D_y',[],'MSD_t0_y',[],'N_opt_y',[]);
velocity_final = struct('v_x',[],'v_y',[]);
...
for dim=1:size(msd_result,2)
    msd_curve = msd_result{1,dim};
    if use_v
        min_data_points = max(fitParam.minimumSkip,4); %need at least 4 points to fit
    else
    ...
    if N_opt>=min_data_points
        [D,msd_t0,v] = fitMSD(msd_curve(1:N_opt,:),fitParam,printParam);
    else
        D = [];
        msd_t0 = [];
        v = [];
    ...
    if dim==1
        D_final.D_x = D;
        D_final.MSD_t0_x = msd_t0;
        D_final.N_opt_x = N_opt;
        velocity_final.v_x = v;
    else
        D_final.D_y = D;
        D_final.MSD_t0_y = msd_t0;
        D_final.N_opt_y = N_opt;
        velocity_final.v_y = v;
    end
end %end dim
