...
experimParam.diffusionCoeffInitial = [0.5]; %double, [µm^2/s] expected diffusion coefficient as starting point of fit. Make array to fit additional D coeff.
experimParam.timestep = 1/200; %double, camera acquisition time in s
experimParam.pixel = 8; %double, width of actual camera pixel in µm
experimParam.magnification = 100; %double, total magnification of microscope
...
trajParam.diffusionGuess = 0.01; %double, minimum sqrt(MSD) of trajectory
...
fitParam.distanceMethod = 'jump'; %string, choose between fitting displacement x,y 'disp' or jump distance sqrt(x.^2+y.^2) 'jump'
...
fitParam.plotFit = true; %boolean, plot switch for fitted histograms
fitParam.breakCondition = [24,9]; %integer, stop fitting if [# of displacements, # of bins] < [m,n]
fitParam.fitAlpha = 0.025; %double, accept higher order if rejecting null hypothesis at confidence level 1-alpha
