...
experimParam.diffusionCoeffInitial = [1]; %double, [µm^2/s] expected diffusion coefficient as starting point of fit. Make array to fit additional D coeff.
experimParam.timestep = 1e-3; %double, camera acquisition time in s
experimParam.pixel = 16; %double, width of actual camera pixel in µm
experimParam.magnification = 16/0.108; %double, total magnification of microscope
...
trajParam.trajMaxLength = 1e3; %integer, maximum allowed length for trajectory
trajParam.frameMaxDistance = 6; %double, maximum distance a molecule travels in pixels per frame
trajParam.diffusionGuess = 0.05; %double, minimum sqrt(MSD) of trajectory
trajParam.inclusionZone = []; %[24,24,5]; %double, exclude all trajectories outside of circle with [middlePoint(1:2),radius]. Just leave as empty to not consider this
...
fitParam.maximumSkip = 16; %integer, maximum time step for trajectory, cannot be longer than the length of longest trajectory minus 1
fitParam.minimumSkip = 4; %integer, minimum time step later used to fit MSD curve. Cannot be smaller than 4
...
fitParam.fitAlpha = 0.05; %double, accept higher order if rejecting null hypothesis at confidence level 1-alpha
fitParam.weightedFit = false; %boolean, take weights into account when MSD curve is fitted
fitParam.fitTrueMSDValues = true; %boolean, when fitting an MSD curve e.g. from displacement histograms and multiple species are allowed, only consider MSD values which are the best outcome of a multivariate histogram fit.
