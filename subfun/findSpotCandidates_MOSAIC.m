...
% find candidates
cands_w0 = detectSpots(img,W0,INTENS_THRSH,PVAL_MIN,calc_bck,plot_enable);
...
cands_w = detectSpots(img,W0-1,INTENS_THRSH,PVAL_MIN,false,false);
...
for i=1:n_cands
    ...
    function [max_indices] = detectSpots(img,w,INTENS_THRSH,PVAL_MIN,calc_bck,plot_enable)
        ...
        if plot_enable
            [pos_y_plot,pos_x_plot] = ind2sub(size(img),max_indices);
            h=figure;
            mim(img);
            hold on
            plot(pos_x_plot,pos_y_plot,'go');
            hold off
            title(gca,'Intensity filtering');
        end
    end %END SUBFUN
